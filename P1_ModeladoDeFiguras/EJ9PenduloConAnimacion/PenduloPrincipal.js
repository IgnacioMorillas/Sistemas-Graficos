...
class PENDULOPRINCIPAL extends THREE.Object3D {
  ...
  createGUI (gui) {
    ...
    folder.add (this.guiControls, 'velocidadPen2', 0, 0.04, 0.001).name ('Velocidad(rad/s) del pendulo : ').listen();
  ...
  update () {
    ...
    if(this.guiControls.animate){
      ...
      if(this.retorno){
        this.guiControls.rotationZ=this.guiControls.rotationZ+ this.guiControls.velocidadPen2;;
      }
      else{
        this.guiControls.rotationZ=this.guiControls.rotationZ- this.guiControls.velocidadPen2;;
      }
